...
const injectStyles = () => {
  ...
  s.textContent = `
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    /* Buttons — identical to FamilySection */
    ...
    /* Nav — identical to FamilySection */
...
// ─── SHARED NAVBAR — pixel-perfect match to FamilySection ────────────────────
function SharedNavbar({ currentPath, onNavigate, onSidebarOpen }) {
  const [mobileMenu, setMobileMenu] = useState(false);
  const isActive = (item) => currentPath === item.path || currentPath === item.key;
  ...
  return (
    <nav style={{
      ...
      <div style={{ display:"flex", alignItems:"center", gap:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:9, cursor:"pointer" }} onClick={() => onNavigate("/analysis")}>
        ...
        <div className="hide-mobile" style={{ display:"flex", gap:2, marginLeft:16 }}>
          {NAV_ITEMS.map(item => (
            <button key={item.key}
              ...
              onClick={() => { onNavigate(item.path); setMobileMenu(false); }}
...
// ─── SIDEBAR — matches FamilySection sidebar exactly ────────────────────────
      ...
      <div style={{
        ...
        <div style={{ marginTop:16 }}>
          <button className="pg-btn pg-btn-family" style={{ width:"100%", justifyContent:"center", marginBottom:8 }}
            onClick={() => { onClose(); onNavigate("/family-section"); }}>
...
export default function ProfilePage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F8F9FA", color:"#212529" }}>
      {/* Background — matches FamilySection */}
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        {/* ─── SIDEBAR ───────────────────────────────────────────────────────── */}
        <Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} onNavigate={(path) => navigate(path)} />
